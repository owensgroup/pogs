...
int ProjectorDirect<T, M>::Init() {
  ...
  cublasOperation_t op_type = block.Rows() > block.Cols()
      ? CUBLAS_OP_T : CUBLAS_OP_N;

  // Compute AA
  if (_A.Order() == MatrixDistDense<T>::ROW) {
    const cml::matrix<T, CblasRowMajor> A =
        cml::matrix_view_array<T, CblasRowMajor>
        (_A.Data(), block.Rows(), block.Cols());
...
int ProjectorDirect<T, M>::Project(const T *x0, const T *y0, T s, T *x, T *y,
  ...
  cml::vector<T> y_vec = cml::vector_view_array(y, block.Rows());
  const cml::vector<T> y0_vec = cml::vector_view_array(y0, block.Rows());
  cml::vector<T> x_vec = cml::vector_view_array(x, block.Cols());
  const cml::vector<T> x0_vec = cml::vector_view_array(x0, block.Cols());
...
template class ProjectorDirect<double, MatrixDistDense<double> >;
template class ProjectorDirect<float, MatrixDistDense<float> >;
